...
namespace Jv.Games.Xna.XForms.Renderers
{
    ...
    public class VisualElementRenderer : ElementRenderer, IControlRenderer
    {
        ...
        protected SizeRequest ContentMeasuredSize { get; private set; }
        #endregion
        ...
        protected virtual SizeRequest MeasureContentOverride(Size availableSize)
        {
            return default(SizeRequest);
        }

        sealed protected override SizeRequest MeasureOverride(Size availableSize)
        {
            if (!Model.IsVisible)
                return MeasuredSize;
